!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! SIM2 CODE: Sea Ice thermo-halodynamic Model CODE !!!!!!!!!!!!!!!!!!!!!!!!!! 
! COPYING 
! 
! Copyright (C) 2007 Letizia Tedesco 
! (letizia.tedesco@cmcc.it) 
! 
...
real(MK) :: A  !<  
real(MK), dimension(:) :: alpha  !< !m2f: check dim(:)!m 
real(MK) :: alpha_i  !<  
real(MK) :: alpha_mi  !<  
real(MK) :: alpha_ow  !<  
real(MK) :: alpha_s  !<  
logical, dimension(6) :: b  !<  
real(MK) :: B  !<  
logical, dimension(5) :: b  !<  
real(MK) :: B  !<  
logical, dimension(5) :: b  !<  
real(MK) :: B  !<  
logical, dimension(5) :: b  !<  
...
real(MK) :: cons  !<  
real(MK) :: cpair  !<  
real(MK), dimension(:) :: cpi  !< !m2f: check dim(:) 
real(MK), dimension(:) :: cpi_bio  !< !m2f: check dim(:) 
real(MK) :: cpi_it  !<  
real(MK) :: cpw  !<  
real(MK) :: cs  !<  
real(MK) :: cwih  !<  
real(MK), dimension(6) :: d  !<  
real(MK), dimension(:) :: delta_bucket  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: delta_sublim  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: deltahi_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: deltahi_melt_surf  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: deltahmi_melt_surf  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: deltahs_melt_surf  !< !m2f: check dim(:)!m 
real(MK) :: deltat  !<  
real(MK), dimension(:) :: ea  !< !m2f: check dim(:) 
real(MK) :: emis  !<  
real(MK) :: emp  !<  
real(MK), dimension(:) :: es  !< !m2f: check dim(:) 
real(MK), dimension(:) :: F  !< !m2f: check dim(:) 
real(MK), dimension(:) :: F1  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: F2  !< !m2f: check dim(:)!m 
real(MK) :: F_it  !<  
real(MK), dimension(:) :: Fbott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Fl  !< !m2f: check dim(:) 
real(MK) :: Fl_it  !<  
real(MK), dimension(:) :: Fla  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Fld  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Flu  !< !m2f: check dim(:) 
real(MK) :: Flu_it  !<  
real(MK), dimension(:) :: Fs  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Fse  !< !m2f: check dim(:) 
real(MK) :: Fse_it  !<  
real(MK), dimension(:) :: Fsurf  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Fw  !< !m2f: check dim(:) 
real(MK) :: gamm  !<  
real(MK), dimension(:) :: h  !< !m2f: check dim(:) 
real(MK) :: h_mix  !<  
real(MK), dimension(:) :: hi  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hi_5  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hi_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hmi  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hmi_new  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hs  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hs_prec  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hs_prec_bucket  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: hs_tot  !< !m2f: check dim(:)!m 
integer, dimension(:) :: I0  !< !m2f: check dim(:)!m 
integer, dimension(:) :: I0_it  !< !m2f: check dim(:)!m 
integer, dimension(:) :: ice_fr  !< !m2f: check dim(:)!m 
integer, dimension(:) :: IM  !< !m2f: check dim(:)!m 
integer, dimension(:) :: IM_it  !< !m2f: check dim(:)!m 
integer :: infra_fr  !<  
integer, dimension(:) :: ISI  !< !m2f: check dim(:)!m 
integer, dimension(:) :: ISI_10  !< !m2f: check dim(:)!m 
integer, dimension(:) :: ISI_10_it  !< !m2f: check dim(:)!m 
integer, dimension(:) :: ISI_bio  !< !m2f: check dim(:)!m 
integer, dimension(:) :: ISI_it  !< !m2f: check dim(:)!m 
integer, dimension(:) :: ISI_layer  !< !m2f: check dim(:)!m 
real(MK) :: k0  !<  
real(MK), dimension(:) :: k0i  !< !m2f: check dim(:) 
real(MK), dimension(:) :: k0i_5_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: k0i_5_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: k0i_bio_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: k0i_ice_5  !< !m2f: check dim(:) 
real(MK), dimension(:) :: k0i_ice_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: k0i_ice_bott  !< !m2f: check dim(:) 
real(MK) :: k_ocean_red  !<  
real(MK) :: k_ocean_vis  !<  
real(MK), dimension(:) :: kb  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kb_5_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kb_5_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kb_bio_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kb_ice_5  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kb_ice_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kb_ice_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Kbott_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: keff  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ki  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ki_5_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ki_5_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ki_bio_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ki_ice_5  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ki_ice_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ki_ice_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: kice_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Kice_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kice_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Kice_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kice_surf  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Kice_surf  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kinterm_ice  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Kinterm_ice  !< !m2f: check dim(:) 
real(MK) :: kmi  !<  
real(MK), dimension(:) :: Kmi  !< !m2f: check dim(:) 
real(MK), dimension(:) :: kmi_av  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: kmi_clear  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: kmi_cloudy  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ks  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Ks  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ks_av  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ks_clear  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ks_cloudy  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ks_daily_severeear  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ks_new  !< !m2f: check dim(:) 
real(MK) :: ks_prec  !<  
real(MK), dimension(:) :: ks_snow  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksi_10_av  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksi_10_clear  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksi_10_cloudy  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksi_av  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksi_clear  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksi_cloudy  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ksnow_ice  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Ksnow_ice  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ksnow_interm  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Ksnow_interm  !< !m2f: check dim(:) 
real(MK) :: kw  !<  
real(MK) :: L0i  !<  
real(MK) :: L0m  !<  
...
real(MK) :: mu  !<  
real(MK) :: mui  !<  
real(MK), dimension(:) :: mui_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: mumi  !< !m2f: check dim(:) 
real(MK), dimension(:) :: mus  !< !m2f: check dim(:) 
real(MK) :: P0  !<  
real(MK), dimension(:) :: Q  !< !m2f: check dim(:)!m 
real(MK) :: q1  !<  
real(MK) :: q2  !<  
real(MK), dimension(:) :: Qi_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Qi_surf  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Qmi  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Qs  !< !m2f: check dim(:) 
real(MK), dimension(:) :: R  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Ri  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ro_br  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ro_br_5  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ro_br_bio  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ro_br_bott  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ro_br_i  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ro_sice_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ro_sice_bulk  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ro_sice_surf  !< !m2f: check dim(:)!m 
real(MK) :: roa  !<  
real(MK), dimension(:) :: roi_av  !< !m2f: check dim(:) 
real(MK) :: roi_bott  !<  
real(MK), dimension(:) :: roi_surf  !< !m2f: check dim(:) 
real(MK) :: romi  !<  
real(MK), dimension(:) :: ros  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: ros_av  !< !m2f: check dim(:) 
real(MK), dimension(:) :: ros_new  !< !m2f: check dim(:)!m 
real(MK) :: ros_prec  !<  
real(MK) :: rosa  !<  
real(MK) :: row  !<  
real(MK), dimension(:) :: Sbr_5  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_5_star  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_bio_star  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_bott_star  !< !m2f: check dim(:)!m 
real(MK) :: Sbr_end  !<  
real(MK), dimension(:) :: Sbr_i  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_i_star  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_ice  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sbr_ice_star  !< !m2f: check dim(:)!m 
character(len=*), dimension(:) :: sea_water_fr  !< !m2f: check dim(:)!m 
character(len=*) :: sen  !<  
real(MK), dimension(:) :: Si  !< !m2f: check dim(:)!m 
character(len=*) :: si_fract  !<  
real(MK), dimension(:) :: Si_mix  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Sice  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sice_5  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sice_5_mix  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sice_av  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Sice_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sice_bio_mix  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sice_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sice_mix  !< !m2f: check dim(:)!m 
character(len=*) :: sigma  !<  
character(len=*), dimension(:) :: snow_fr  !< !m2f: check dim(:)!m 
character(len=*) :: ss_fract  !<  
real(MK) :: Ssnow  !<  
real(MK), dimension(:) :: Ssnowice  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Ssnowice_new  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Sw  !< !m2f: check dim(:)!m 
real(MK), dimension(:,:) :: T  !< !m2f: check dim(:,:) 
real(MK), dimension(:) :: T0  !< !m2f: check dim(:)!m 
real(MK) :: T0_iter  !<  
real(MK), dimension(:) :: T0_star  !< !m2f: check dim(:)!m 
real(MK) :: T_melt  !<  
real(MK) :: T_melt_2  !<  
real(MK), dimension(:) :: T_prev  !< !m2f: check dim(:) 
real(MK), dimension(3) :: T_prev_melt  !< !m2f: check dim(:) 
real(MK), dimension(4) :: T_prev_melt_2  !<  
real(MK) :: Tb  !<  
real(MK) :: Tfr  !<  
real(MK) :: Tfrs  !<  
real(MK), dimension(:) :: Ti  !< !m2f: check dim(:)!m 
real(MK) :: Ti_iter  !<  
real(MK), dimension(:) :: Ti_star  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Tice  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Tice_5  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Tice_av  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Tice_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Tmix  !< !m2f: check dim(:)!m 
real(MK) :: Tmix_star  !<  
real(MK), dimension(:) :: Ts  !< !m2f: check dim(:)!m 
real(MK) :: Ts_iter  !<  
real(MK), dimension(:) :: Ts_star  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Tsi  !< !m2f: check dim(:)!m 
real(MK) :: Tsi_iter  !<  
real(MK), dimension(:) :: Tsi_star  !< !m2f: check dim(:)!m 
real(MK) :: Va  !<  
real(MK), dimension(:) :: Vbr_5  !< !m2f: check dim(:) 
real(MK), dimension(:) :: Vbr_bio  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Vbr_bott  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Vbr_i  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: Vbr_ice  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: vi  !< !m2f: check dim(:)!m 
real(MK), dimension(:) :: viola  !< !m2f: check dim(:) 
real(MK), dimension(:) :: vis_fr  !< !m2f: check dim(:) 
! 
...
c0=2093.0
! (J (kg*??deg)-1) specific heat of fresh ice
c1=21.875
! (K) emp.costant
c2=7.66
! (K) emp.costant
...
cail=1.75*10**-3
! (non-dim) bulk transfer coefficient
cais=1.20*10**-3
! (non-dim) bulk transfer coefficient
cpair=1004.0 ! (J kg**-1 K**-1) air heat capacity
cpw=4186.0 ! (J kg**-1 K**-1) specific heat of sea water
cs=2093.0
! (J (kg K)-1) specific heat of snow
cwih=2.8*10**-4
! (non-dim) bulk heat trans coeff (after Omstedt and Wettlaufer,1992)
deltat=dt ! (s) time step 1.5 h
emis=0.97
! (non-dim) water/ice emissivity
...
kmi=0.9
! (W m**-1 K**-1) snow ice thermal conductivity (after Lepparanta, 1983)
...
mu=0.054
! (??C) ratio between Tfr and brines salinity (after Assur, 1958)
P0=1013.0
! (mbar) seawater pressure at the surface
...
sigma=5.68*10**-8
! (W m**-2 K) Stefan-Boltzmann constant
...
Tfrs=273.15
! (K) freezing temp of snow/fresh water
...
Tmix(1)=271.34 ! Initial mixed layer temperature
...
Sw(1)=34.1552 ! Seawater salinity

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! START RUNNING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 

do i=2,nmax ! maximum number of timestep 
Sw(i)=32.0 ! Seawater salinity
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
! SNOW DENSITY 
...
if ((hs(i-1)+hs_prec_bucket(i))>0.1) then 
alpha(i)=alpha_s
else if (hmi(i-1)>hmi_min) then 
alpha(i)=alpha_mi
else if (hi(i-1)>hi_min) then 
...
! TEMPERATURE 
if ((hs(i-1)+hs_prec_bucket(i))>hs_min) then 

! SEA ICE MODEL ON/SLAB OCEAN OFF 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
if (hmi(i-1)>hmi_min) then 
...
!m2f: b=[ ((-F_it-I0_it)/(la+sen+lat+cons)),(I0_it*mus(i)),(IM_it*mumi(i)),(mui.*Kice_bott(i)*Tfr,+,mui.*ISI_it) ] '
b=[ ((-F_it-I0_it)/(la+sen+lat+cons)),(I0_it*mus(i)),(IM_it*mumi(i)),(mui.*Kice_bott(i)*Tfr,+,mui.*ISI_it) ] transpose()
!m2f: T_prev=[ T0_star(j-1),Ts_star(1),Tsi_star(1),Ti_star(1) ] '
T_prev=[ T0_star(j-1),Ts_star(1),Tsi_star(1),Ti_star(1) ] transpose()
T(:,j)=A*T_prev + b
T0_star(j)=T(1,j)
Ts_star(j)=T(2,j)
Tsi_star(j)=T(3,j)
...
T0_iter=T0_star(size(XXX,XXX))
Ts_iter=Ts_star(size(XXX,XXX))
Tsi_iter=Tsi_star(size(XXX,XXX))
Ti_iter=Ti_star(size(XXX,XXX))
Tmix_star=Tfr
...
!m2f: c=[ 0,(I0_it*mus(i)),(IM_it*mumi(i)),(mui.*Kice_bott(i)*Tfr,+,mui.*ISI_it) ] '
c=[ 0,(I0_it*mus(i)),(IM_it*mumi(i)),(mui.*Kice_bott(i)*Tfr,+,mui.*ISI_it) ] transpose()
!m2f: T_prev_melt=[ Tfr,Ts(i-1),Tsi(i-1),Ti(i-1) ] '
T_prev_melt=[ Tfr,Ts(i-1),Tsi(i-1),Ti(i-1) ] transpose()
...
!m2f: d=[ 0,0,(IM_it*mumi(i)),(mui.*Kice_bott(i)*Tfr,+,mui*ISI_it) ] '
d=[ 0,0,(IM_it*mumi(i)),(mui.*Kice_bott(i)*Tfr,+,mui*ISI_it) ] transpose()
!m2f: T_prev_melt_2=[ Tfr,Tfr,Tsi(i-1),Ti(i-1) ] '
T_prev_melt_2=[ Tfr,Tfr,Tsi(i-1),Ti(i-1) ] transpose()
...
!m2f: b=[ ((-F_it-I0_it)/(la+sen+lat+cons)),(I0_it*mus(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] '
b=[ ((-F_it-I0_it)/(la+sen+lat+cons)),(I0_it*mus(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] transpose()
!m2f: T_prev=[ T0_star(j-1),Ts_star(1),Ti_star(1) ] '
T_prev=[ T0_star(j-1),Ts_star(1),Ti_star(1) ] transpose()
T(:,j)=A*T_prev + b
T0_star(j)=T(1,j)
Ts_star(j)=T(2,j)
Ti_star(j)=T(3,j)
...
!m2f: c=[ 0,(I0_it*mus(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] '
c=[ 0,(I0_it*mus(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] transpose()
!m2f: T_prev_melt=[ Tfr,Ts(i-1),Ti(i-1) ] '
T_prev_melt=[ Tfr,Ts(i-1),Ti(i-1) ] transpose()
T_melt=B*T_prev_melt + c
Ts_iter=T_melt(2)
if (Ts_iter<Tfr) then 
Ti_iter=T_melt(3)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
else if ((hs(i-1)+hs_prec_bucket(i))<=hs_min) then 
...
!m2f: b=[ ((-F_it-I0_it)/(la+sen+lat+conmi)),(I0_it*mumi(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] '
b=[ ((-F_it-I0_it)/(la+sen+lat+conmi)),(I0_it*mumi(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] transpose()
!m2f: T_prev=[ T0_star(j-1),Tsi_star(1),Ti_star(1) ] '
T_prev=[ T0_star(j-1),Tsi_star(1),Ti_star(1) ] transpose()
T(:,j)=A*T_prev + b
T0_star(j)=T(1,j)
Tsi_star(j)=T(2,j)
Ti_star(j)=T(3,j)
...
!m2f: c=[ 0,(I0_it*mumi(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] '
c=[ 0,(I0_it*mumi(i)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it) ] transpose()
!m2f: T_prev_melt=[ Tfr,Tsi(i-1),Ti(i-1) ] '
T_prev_melt=[ Tfr,Tsi(i-1),Ti(i-1) ] transpose()
T_melt=B*T_prev_melt + c
Tsi_iter=T_melt(2)
if (Tsi_iter<Tfr) then 
Ti_iter=T_melt(3)
...
!m2f: b=[ ((-F_it-I0_it)/(la+sen+lat+coni)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it),0 ] '
b=[ ((-F_it-I0_it)/(la+sen+lat+coni)),(mui.*Kice_bott(i).*Tfr,+,mui.*ISI_it),0 ] transpose()
!m2f: T_prev=[ T0_star(j-1),Ti_star(1),Ti_star(1) ] '
T_prev=[ T0_star(j-1),Ti_star(1),Ti_star(1) ] transpose()
T(:,j)=A*T_prev + b
T0_star(j)=T(1,j)
Ti_star(j)=T(2,j)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
if (abs(T0_star(j)-T0_star(j-1))< 0.01, exit) then ! CONVERENCE CRITERION: 0.01 K
end if 
end do 

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! END ITERATION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
if (length (T0_star)>20) then 
'WARNING: ITERATION >20'
end if 
if (length (T0_star)==100) then ! MAX NUMBER OF ITERATIONS FOR CONVERGENCE
'NOT CONVERGENT'
end if 

if (max(shape(T0_star))==100, return) then ! STOP THE SEA ICE MODEL IF NOT CONVERGENT
end if 
...
if ((hs(i-1)+hs_prec_bucket(i)-delta_sublim(i))> ((hi(i-1).*(row-ro_sice_surf(i-1)*10**3)/ros_av(i)) + (hmi(i-1)*(row-romi)/ros_av(i)))) then ! SNOW ICE FORMATION
...
if ((hs(i-1) + hs_prec_bucket(i) - delta_sublim(i))> (hi(i-1).*(row-ro_sice_surf(i-1)*10**3)/ros_av(i) - hs_prec_bucket(i).*ros_new(i)/ros(i))) then ! SNOW ICE FORMATION
...
if (Vbr_ice(i-1)> 0.05) then ! DESALINATION
...
else if (Vbr_ice(i-1)<=0.05) then ! NO DESALINATION
Sice_mix(i)=((hi(i)-hi(i-1)).*Sice_bott(i) + hi(i-1).*Sice(i-1))/hi(i)
Sice(i)=Sice_mix(i)
Sbr_ice(i)=-Tice(i)/mu
Vbr_ice(i)=Sice(i)./Sbr_ice(i)
...
else if (Vbr_ice(i-1)<0.05) then ! NO FLUSHING
Q(i)=0.0
Sice(i)=Sice(i-1)
Sbr_ice(i)=-Tice(i)/mu
Vbr_ice(i)=Sice(i)./Sbr_ice(i)
